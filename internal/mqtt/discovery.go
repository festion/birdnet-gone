...
func (p *Publisher) publishSourceDiscovery(ctx context.Context, source datastore.AudioSource, settings *conf.Settings) error {
	...
	if err := p.publishSensor(ctx, nodeID, sourceID, SensorConfidence, &DiscoveryPayload{
		...
		StateTopic:        p.config.BaseTopic,
		ValueTemplate:     fmt.Sprintf("{{ (value_json.Confidence * 100) | round(1) if value_json.sourceId == '%s' else (this.state if this.state not in ['unknown', 'unavailable'] else none) }}", source.ID),
		UnitOfMeasurement: "%",
	...
	if settings.Realtime.Audio.SoundLevel.Enabled {
		if err := p.publishSensor(ctx, nodeID, sourceID, SensorSoundLevel, &DiscoveryPayload{
			...
			ValueTemplate:     fmt.Sprintf("{{ value_json.b['1.0_kHz'].m if value_json.src == '%s' else (this.state if this.state not in ['unknown', 'unavailable'] else none) }}", source.ID),
