...
func TestNightFilterExcludesSunriseSunsetWindows(t *testing.T) {
	...
	//    parses filters.DateStart with time.Parse("2006-01-02", ...) which
	//    returns UTC midnight. SunCalc caches by the same date.Equal() check,
	//    and astral.julianday() does date.UTC() and uses the UTC calendar day
	//    components. If the test passes a Local-midnight date, astral computes
	//    sunrise/sunset for a *different UTC calendar day* than the filter
	//    does (in NZST, Local-midnight is at noon UTC of the previous UTC day),
