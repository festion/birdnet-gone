...
- **Right — probe amplitude directly.** Subscribe to `birdnet/soundlevel` on the broker (creds from `realtime.mqtt` in `/home/jeremy/.config/birdnet-go/config.yaml`, as the health script does) and read the band means. Payload keys are **short**: `{"ts","node","src","nm","dur","b":{"<band>":{"f","n","x","m"}}}` — the per-band mean is **`m`**, not `mean` (a parser looking for `mean` silently reports 0 bands). Healthy here is a median around **−85 to −90 dB** across 30 bands; desync is a flat **−200.0 dB** (digital silence). Confirmed at 11:43/11:45 (median −87.3 / −85.8) *before* the watchdog's own 11:48 `audio recovered after 2h45m of silence` / `median amp -87.6 dB` — the independent probe and the watchdog agreed to within 2 dB.
- **`arecord -l` is not a health signal either:** it showed the card present with `Subdevices: 0/1` (in use by birdnet-go) throughout the entire 2h45m outage. Device enumerated + held open says nothing about whether audio is flowing.
- **Watchdog behaviour worth knowing:** restart backoff is 1h per episode, so during a long desync most ticks read `restart HELD`; and after ~9 failed restarts the self-heal is simply not going to fix it — the fault is the wireless link, and only a physical re-pair clears it.
- **Trend to watch:** re-pairs on 07-21 and 07-26 (5 days apart) while the daily detection *window* shrank from ~8h (07-17, 159 detections) to zero (07-24). A fix that must be reapplied on a shortening interval is a degrading component — see #2097 (shade test + ESP32-C6 swap).
